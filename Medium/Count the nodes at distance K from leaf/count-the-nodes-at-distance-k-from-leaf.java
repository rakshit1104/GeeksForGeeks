...
class Solution
{
    ...
    int printKDistantfromLeaf(Node root, int k)
    {
        ...
        if(isValid(root,0,k)){
            return printKDistantfromLeaf(root.left,k)+printKDistantfromLeaf(root.right,k) + 1;
        }
        
        return printKDistantfromLeaf(root.left,k)+printKDistantfromLeaf(root.right,k);
    }
    
    boolean isValid(Node node, int d, int k){
            if(node==null) return false;
            if(d>k) return false;
            if(node.left==null&&node.right ==null&&d==k)
            return true;
            return isValid (node.left,d+1,k) || isValid(node.right,d+1,k);
    }
}